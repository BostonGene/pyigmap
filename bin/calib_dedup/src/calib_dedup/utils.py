...
def run_command(command: list[str] | str, stdin=None, shell=False):
...
def generate_consensus(
    in_fq1: str, in_fq2: str, cluster_file: str, min_reads_per_cluster: int, max_reads_per_cluster: int
) -> tuple[str, str]:
    ...
    with tempfile.NamedTemporaryFile(delete=False) as f1, tempfile.NamedTemporaryFile(delete=False) as f2:
        fq1_cons_prefix, fq2_cons_prefix = f1.name, f2.name
    calib_cons_cmd = [
        'calib_cons',
        '--fastq',
        in_fq1,
        ...
        str(min_reads_per_cluster),
        '--max-reads-per-cluster',
        str(max_reads_per_cluster),
    ]
...
def check_error_tolerance_size(umi_len: int, error_tolerance: int):
    if umi_len and error_tolerance > umi_len:
        logger.critical(
            f'Error tolerance size (={error_tolerance}) should be no larger '
            f'than the umi length (={umi_len}), exiting...'
        )
        sys.exit(1)
