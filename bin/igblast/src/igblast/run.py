...
IGBLAST_DIR = os.environ.get('IGBLAST_DIR', '/igblast')

TEMPDIR_NAME = os.environ.get('TEMPDIR', '/tmp/')
OUTPUT_FOLDER_NAME = os.environ.get('HOME', '')

FASTA_CHUNKS_DIR = os.path.join(TEMPDIR_NAME, os.path.dirname(tempfile.TemporaryDirectory().name))

RECEPTOR_GLOSSARY = {'BCR': ['Ig'], 'TCR': ['TCR'], 'all': ['Ig', 'TCR']}
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        '--receptor',
        help="Receptor type: 'BCR', 'TCR' or 'all'",
        choices=['BCR', 'TCR', 'all'],
        type=str,
        required=True,
    )
    ...
    parser.add_argument(
        '--reads-chunk-size', help='Count of sequences processed in one run of IgBLAST', type=int, default=50_000
    )
    parser.add_argument(
        '--out-annotation',
        help='Output AIRR-formatted annotation table',
        required=True,
...
def read_seq_gz_file(file_path: str) -> str:
    ...
    pigz_process = run_and_check_with_message(pigz_cmd, 'pigz', return_proc=True, capture_output=True, stderr=None)
    logger.info(f'{file_path} file is successfully read.')

    if pigz_process is None or pigz_process.stdout is None:
...
def convert_fastq_to_fasta(fastq_stdin: str) -> str:
    ...
    seqtk_process = run_and_check_with_message(
        seqtk_cmd, 'seqtk', return_proc=True, capture_output=True, stderr=None, input=fastq_stdin
    )
    logger.info('Converting FASTQ -> FASTA successfully done.')

    if seqtk_process is None or seqtk_process.stdout is None:
        logger.critical('Failed to convert FASTQ to FASTA')
        sys.exit(1)
...
def run_igblast(seq_file: str, receptor: str) -> str:
    ...
    with tempfile.NamedTemporaryFile(suffix='.tsv', delete=False) as f:
        output_annotation = os.path.join(TEMPDIR_NAME, os.path.basename(f.name))
    ...
    igblast_cmd = [
        ...
        seq_file,
        '-germline_db_V',
        os.path.join(IGBLAST_DIR, 'database', f'human.{receptor}.V'),
        '-germline_db_D',
        os.path.join(IGBLAST_DIR, 'database', f'human.{receptor}.D'),
        '-germline_db_J',
        os.path.join(IGBLAST_DIR, 'database', f'human.{receptor}.J'),
        '-organism',
        'human',
        '-c_region_db',
        os.path.join(IGBLAST_DIR, 'database', 'ncbi_human_c_genes'),
        '-auxiliary_data',
        os.path.join(IGBLAST_DIR, 'optional_file', 'human_gl.aux'),
        '-ig_seqtype',
        receptor,
        '-show_translation',
        '-outfmt',
...
def concat_annotations(annotation_files: list[str]) -> str:
    ...
    with tempfile.NamedTemporaryFile(suffix='.tsv', delete=False) as f:
        out_annotation_path = os.path.join(TEMPDIR_NAME, os.path.basename(f.name))
    with gzip.open(out_annotation_path, 'ab') as out_annotation_obj:
        for i, annotation_file in enumerate(annotation_files):
            with open(annotation_file) as annotation_obj:
                if i > 0:
