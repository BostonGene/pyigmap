...
from cdr3nt_error_corrector import airr
from cdr3nt_error_corrector import filter as cdr3_filter
from cdr3nt_error_corrector.correct import ClonotypeCorrector
from cdr3nt_error_corrector.logger import set_logger
from cdr3nt_error_corrector.pgen import PgenModel
...
def get_filtered_annotation(annotation: pd.DataFrame, top_c_call: bool, top_v_alignment_call: bool,
    ...
    for annotation_by_locus, locus in zip(*airr.split_by_loci(annotation), strict=False):
        ...
        if not skip_pgen_calculation:
            ...
            corrected_annotation.loc[:, 'pgen'] = pgen_model.get_pgen(corrected_annotation['junction_aa'].tolist())  # type: ignore[index]
    ...
    return cdr3_filter.run_filtration(
