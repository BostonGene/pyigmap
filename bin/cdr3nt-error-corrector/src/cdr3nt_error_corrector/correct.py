...
class ClonotypeCorrector:
    ...
    def aggregate_clonotypes(self, annotation: pd.DataFrame, grouping_columns: list[str]) -> DataFrame:
    ...
    def _make_counters(self, annotation: pd.DataFrame) -> tuple[list, dict]:
        ...
        # List sorted from largest to smallest, needed for _update_counters_inplace
        clonotype_count_list = []

        for clonotype, count in zip(
            annotation[CLONOTYPE_COLUMNS].values, annotation[COUNT_COLUMN].values, strict=False
        ):
            v_call, j_call, junction = clonotype
            counter = ClonotypeCounter(v_call, j_call, junction, count, self.error_rate)
