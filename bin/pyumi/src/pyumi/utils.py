...
from pyumi.extract import get_processed_fastqs
...
def run_command(command: list[str], stdin=None, stdout=False) -> str | None:
...
def split_by_chunks(fq1_path: str, fq2_path: str) -> list[str]:
    ...
    return list(zip(fq1_chunks, fq2_chunks, strict=False))  # type: ignore[return-value]
