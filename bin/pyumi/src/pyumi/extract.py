...
import pyfastx  # type: ignore[import-untyped]
import regex
...
def get_processed_reads(
    ...
    result = tuple(map(list, zip(*processed_reads, strict=False)))
...
def get_fastq_reads(in_fq1_path: str, in_fq2_path: str) -> tuple[list[str], list[str]]:
    ...
    reads1 = list(pyfastx.Fastq(in_fq1_path, build_index=False, full_name=True))
    reads2 = list(pyfastx.Fastq(in_fq2_path, build_index=False, full_name=True))
