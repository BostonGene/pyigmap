...
from metadata import FileSource, Experiment, Run
...
def run_and_check_with_message(
    ...
    **subprocess_args
) -> Optional[Union[CompletedProcess[str], CompletedProcess[Any], CalledProcessError]]:
    """
...
def save_outputs(fq1_runs: List[str], fq2_runs: List[Optional[str]], out_fq1: str, out_fq2: Optional[str]) -> None:
...
def convert_sra_to_fastq(sra_file: str, accession: str, ngc: Optional[str] = None) -> tuple[str, Optional[str]]:
    """
...
def download_file(url: str, download_retries: int) -> str:
    out = tempfile.NamedTemporaryFile(dir=TEMPDIR).name
...
def handle_run_files(run: Run, download_retries: int) -> tuple[List[str], List[Optional[str]]]:
    """
...
def process_experiment(identifier: str, download_retries: int) -> tuple[List[str], List[Optional[str]]]:
    """
...
def process_run(identifier: str, download_retries: int) -> tuple[List[str], List[Optional[str]]]:
    """
