...
from fastp.logger import set_logger
...
def process_reads_chunk_in_parallel(
    fq1_reads_chunk_list: list[FastqRead],
    fq2_reads_chunk_list: list[FastqRead],
    inner_distance_size: int,
) -> tuple[bytes, ...]:
    """Merges read chunk in parallel"""
    with multiprocessing.Pool(processes=CPU_COUNT) as pool:
        processed_reads_chunk = pool.starmap(
            mock_merge_one_reads_pair, zip(fq1_reads_chunk_list, fq2_reads_chunk_list, repeat(inner_distance_size))
        )
    return tuple(read.encode() for read in processed_reads_chunk)


def append_merged_reads_to_fastq_file(
    out_fq12_file_obj: gzip.GzipFile, mock_merged_read_chunk: tuple[bytes, ...]
) -> None:
    """Append mock merged reads into fq12 file"""
    out_fq12_file_obj.writelines(mock_merged_read_chunk)


def mock_merge_by_chunks(
    fq1_path: str, fq2_path: str, inner_distance_size: int, reads_chunk_size: int, out_fq12: str
) -> None:
    ...
    with (
        gzip.open(fq1_path, 'rt') as fq1_file_obj,
        ...
        while fq1_reads_chunk_list and fq2_reads_chunk_list:
            mock_merged_read_chunk = process_reads_chunk_in_parallel(
                fq1_reads_chunk_list, fq2_reads_chunk_list, inner_distance_size
            )
