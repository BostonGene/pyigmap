...
from mock_merge_rs import mock_merge_by_chunks  # type: ignore[import-untyped]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        '--disable-filters',
        help='Mode(s) to disable',
        choices=['length_filtering', 'quality_filtering', 'adapter_trimming'],
        nargs='+',
        default=[],
        action='extend',
    )
...
def generate_temp_fastq_files() -> tuple[str, str, str]:
    ...
    for _ in range(3):
        with tempfile.NamedTemporaryFile(suffix='.fastq', delete=False) as f:
            files.append(os.path.join(TEMPDIR_NAME, os.path.basename(f.name)))
    return files[0], files[1], files[2]


def run_fastp(
    in_fq1: str, in_fq2: str | None, disable_filters: list[str], merge: bool, json: str, html: str
) -> tuple[str, str, str]:
...
def compress_to_gzip(src_file: str, dst_file: str):
    with open(dst_file, 'w') as f_out:
        run_and_check_with_message(['pigz', '-c', src_file], 'pigz', stdout=f_out)
    logger.info(f"Compressed '{src_file}' to '{dst_file}' successfully.")


def save_final_fastq(
    merge_reads: bool,
    mock_merge_reads: bool,
    paired_end: bool,
    fq1_src: str,
    fq2_src: str,
    fq12_src: str,
...
def main() -> None:
    ...
    tmp_fq1, tmp_fq2, tmp_fq12 = run_fastp(
        args.in_fq1, args.in_fq2, args.disable_filters, run_reads_merging, args.json, args.html
    )
