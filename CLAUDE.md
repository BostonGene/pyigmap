...
- `bin/` - Python tools and their Dockerfiles:
  ...
  - `downloader/` - Public database fetching
...
- `run.py` - CLI entrypoint with argparse
- `utils.py` - Core logic functions
- `logger.py` - Logging setup
- `Dockerfile` - Multi-stage build (reference builder + tool image)
- `pyproject.toml` - Python dependencies and metadata (uses uv for dependency management)
...
- Tools without Python dependencies (binary wrappers) have minimal pyproject.toml files
...
1. Create `bin/newtool/` directory with run.py, utils.py, logger.py, Dockerfile
2. Create `pyproject.toml` with dependencies (requires-python = ">=3.12")
3. Run `uv lock` in the tool directory to generate `uv.lock`
4. Add Nextflow process in `modules/local/newtool.nf` with appropriate container and resource labels
5. Integrate process into relevant subworkflow in `subworkflows/`
6. Add unit tests in `bin/newtool/unit_tests/`
7. Add workflow test in `tests/newtool/test_newtool.yml`
8. Build images: `just build-step-image newtool image`
